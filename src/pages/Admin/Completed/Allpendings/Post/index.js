...
function Post({ name, postID, descriptions, cancel, category, clientID, dueDate, features, images, orderID, paidAmount, paidStatus, progress, projectLinks, projectStatus, webLinks, subCategory, subList, techies, timestamp, totalAmount}) {
  ...
  const acceptFun = () => {
    const confirmation = window.confirm(
      `Are you sure you want to complete ${name} - orderID: ${orderID}?`
    );
      ...
      db.collection('orders')
        ...
        .then(() => {
          ...
          Swal.fire({
            icon: 'success',
            title: 'Completed!',
            text: `${name} - ${orderID} is now completed`,
          })
        })
        .catch((err) => {
          toast.error(`Failed to complete ${name} - ${orderID}`, {
            position: 'top-center',
          });
        });
    }
  ...
  const sendAcceptEmail = () => {
    const email = `${currentUser?.email}`; // Replace with the email address you want to send the email to
    const subject = `Completed Order ID:${orderID?.toString()} - ${name}`; // Replace with the subject of your email
...
Sub Category: ${subCategory}
    ...
    const body = `
Dear ${currentUser?.firstName} ${currentUser?.lastName}, Hope This Email Finds You Well. I am Glad To Inform You That Your Order Has Been completed. You can go to your progress tab under Product and Project Links. Below Are Summary Of Your Order:
...
${messageMarkdown}
  ...
  return (
    <TableRow>
      ...
      <TableCell align='right'>{projectStatus === "Pending" ? <span style={{color:'#FF5C5C',fontWeight:'bold',cursor:'pointer'}} onClick={acceptFun}>Pending</span>: projectStatus === "Accepted" ? <span style={{color:'#2EFF2E',fontWeight:'bold'}}>Accepted</span>: projectStatus === "Ongoing" ? <span onClick={acceptFun} style={{color:'#D396FF',fontWeight:'bold',cursor:'pointer'}}>Ongoing</span> : <span style={{color:'#FFB52E',fontWeight:'bold'}}>Completed</span>}</TableCell>
...
>
  ...
  >
    ...
    <TabPanel value={value} index={1} dir={theme.direction}>
      <Progress name1={name} orderID={orderID} clientID={clientID} totalAmount={totalAmount} dueDate={dueDate} paidAmount={paidAmount} paidStatus={paidStatus} progress={progress} projectLinks={projectLinks} webLinks={webLinks} subList={subList} projectStatus={projectStatus} postID={postID}/>
    </TabPanel>
