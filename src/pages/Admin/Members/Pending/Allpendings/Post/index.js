...
function Post({ name, postID, descriptions, cancel, category, clientID, dueDate, features, images, orderID, paidAmount, paidStatus, progress, projectLinks, projectStatus, webLinks, subCategory, subList, techies, timestamp, totalAmount}) {
  ...
  const acceptFun = () => {
    const confirmation = window.confirm(
      `Are you sure you want to accept ${name} - orderID: ${orderID}?`
    );
      ...
      db.collection('orders')
        ...
        .then(() => {
          ...
          Swal.fire({
            icon: 'success',
            title: 'Accepted!',
            text: `${name} - ${orderID} is now ongoing`,
          })
        })
        .catch((err) => {
          toast.error(`Failed to accept ${name} - ${orderID}`, {
            position: 'top-center',
          });
        });
    }
  ...
  const sendAcceptEmail = () => {
    ...
    const subject = `ACCEPTED Order ID:${orderID?.toString()} - ${name}`; // Replace with the subject of your email
...
Techies: ${techies}
    ...
    const body = `
Dear ${currentUser?.firstName} ${currentUser?.lastName}, Hope This Email Finds You Well. I am Glad To Inform You That Your Order Has Been Accepted And Is Now Ongoing. Below Are The Details Of Your Order:
...
${messageMarkdown}
  ...
  return (
    <TableRow>
      ...
      <TableCell align='right'>{projectStatus === "Pending" ? <span style={{color:'#FF5C5C',fontWeight:'bold',cursor:'pointer'}} onClick={acceptFun}>Pending</span>: projectStatus === "Accepted" ? <span style={{color:'#2EFF2E',fontWeight:'bold'}}>Accepted</span>: projectStatus === "Ongoing" ? <span style={{color:'#D396FF',fontWeight:'bold'}}>Ongoing</span> : <span style={{color:'#FFB52E',fontWeight:'bold'}}>Completed</span>}</TableCell>
